...
pub fn build(b: *std.Build) void {
    ...
    const quantum_mod = b.addModule("quantum", .{
        .root_source_file = std.Build.LazyPath{ .path = "../mod.zig" },
        .target = target,
        .optimize = optimize,
    });

    const glimmer_mod = b.addModule("glimmer", .{
        .root_source_file = std.Build.LazyPath{ .path = "../../glimmer/mod.zig" },
        .target = target,
        .optimize = optimize,
    });

    const tests = b.addTest(.{
        .root_source_file = std.Build.LazyPath{ .path = "tests/main_test.zig" },
        .target = target,
        .optimize = optimize,
    });
